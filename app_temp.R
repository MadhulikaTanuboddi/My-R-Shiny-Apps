...
#python_packages <- read.csv(here("My-Shiny-App", "600_Python_Packages.csv"))
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      #numericInput("num_version", label = "Numeric input", value = 1, min = 0, max = 2020)),
      
      # radioButtons("radio_dist", "Distribution:",
...
server <- function(input, output, session) {
  ...
  output$table <- renderDT(
    python_dist1() %>% select(package_name, version, summary, distribution)
  )
  
  output$histPlot <- renderPlot({
    #hist(python_dist1()$distribution)
    #plot (as.factor(python_packages$distribution))
    plot (as.factor(python_dist1()$distribution))
